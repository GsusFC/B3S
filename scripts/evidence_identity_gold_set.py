#!/usr/bin/env python3
"""Evaluate or prepare the single-reviewer evidence identity gold set."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Evaluate reviewed evidence identity cases without runtime authority."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    if args.write_review_template:
        if args.write_review_template.exists():
            parser.error(
                f"review template already exists: {args.write_review_template}"
            )
    ...
    rendered = (
        ...
        if args.format == "json"
        else render_identity_gold_set_markdown(result)
    )
    if args.output:
