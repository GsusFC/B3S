#!/usr/bin/env python3
"""Evaluate or prepare the real claim-to-tile mapping review set."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate real evidence-to-claim-to-tile mappings without "
    ...
    parser.add_argument(
        "--reviews",
        type=Path,
        default=DEFAULT_REVIEW_ROOT / "reviews.jsonl",
    )
    parser.add_argument(
        "--format",
    ...
    rendered = (
        ...
        + "\n"
        if args.format == "json"
        else render_claim_tile_review_set_markdown(result)
    )
    if args.output:
