...
- Owned page selection combines observed navigation with same-domain URLs published through robots/sitemaps, keeps an eight-page budget, prioritizes role diversity and additional recognized strategic pages before using up to two remaining slots for sitemap-only exploration, filters disallowed/resource URLs, and persists `owned-page-selection-v3` with discovery source, navigation status, strategic role, capture outcome, `lastmod`, and the visited/known coverage denominator.
...
`022_evidence_vault_raw_incremental_planning.sql`, is idempotent, and
