CREATE TABLE b3s_history.evidence_claim_reconciliation_events (
    id uuid PRIMARY KEY,
    brand_id uuid NOT NULL REFERENCES b3s_history.brands(id) ON DELETE RESTRICT,
    subject_type text NOT NULL CHECK (subject_type = 'claim_relation'),
    subject_id text NOT NULL CHECK (subject_id ~ '^[0-9a-f]{64}$'),
    relation_type text NOT NULL CHECK (
        relation_type IN ('coexistence_candidate', 'replacement_candidate')
    ),
    ...
    ) REFERENCES b3s_history.evidence_claim_reconciliation_events (
...
CREATE INDEX idx_b3s_claim_reconciliation_subject
    ON b3s_history.evidence_claim_reconciliation_events (
        brand_id, subject_type, subject_id, sequence DESC
    );

CREATE INDEX idx_b3s_claim_reconciliation_created
    ON b3s_history.evidence_claim_reconciliation_events (
