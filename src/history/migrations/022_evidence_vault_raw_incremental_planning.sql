-- Freeze the exact worker-local incremental planning parent and history.
--
...
BEGIN
    ...
    RETURN jsonb_build_object(
        ...
        -- Exact accepted relation translation is optional and fail-closed.  It
        -- must never treat operational evidence_id as a capture fingerprint.
        'accepted_evidence_tile_relations', '[]'::jsonb,
...
TO b3s_history_vault_provenance_owner;
ALTER FUNCTION b3s_history.read_evidence_vault_raw_planning_context(
    text, text, text, uuid, uuid, uuid
...
COMMENT ON FUNCTION b3s_history.read_evidence_vault_raw_planning_context(
    ...
    'Scanner-worker-only bounded planning projection; freezes canonical parent and prior analyzed evidence under the raw append lock order.';
