CREATE TABLE b3s_history.evidence_scoring_recovery_review_events (
    id uuid PRIMARY KEY,
    brand_id uuid NOT NULL REFERENCES b3s_history.brands(id) ON DELETE RESTRICT,
    subject_type text NOT NULL CHECK (subject_type = 'scoring_recovery'),
    ...
    candidate_fingerprint text NOT NULL CHECK (
        ...
        AND candidate_fingerprint = subject_id
    ),
    ...
    ) REFERENCES b3s_history.evidence_scoring_recovery_review_events (
...
CREATE INDEX idx_b3s_scoring_recovery_review_subject
    ON b3s_history.evidence_scoring_recovery_review_events (
        brand_id, subject_type, subject_id, sequence DESC
    );

CREATE INDEX idx_b3s_scoring_recovery_review_created
    ON b3s_history.evidence_scoring_recovery_review_events (
