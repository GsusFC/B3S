...
def test_checkpoint_migration_contract_is_forward_only_and_non_authoritative():
    ...
    prior = migrations["035_evidence_vault_sv9_empty_relation_witness.sql"]
    relaxed = migrations["037_evidence_vault_sv9_empty_witness_plan_relaxation.sql"]
    normalized = relaxed.replace(
        "evidence_vault_sv9_judgment_candidate_witness_v3_check",
        "evidence_vault_sv9_judgment_candidate_empty_witness_check",
    ).replace(
        "-- Permit a canonical empty delta-projection set for first-run empty witnesses.\n",
        "",
    )
    removed = "                        AND candidate_payload->'plan'->'delta_projections' <> '[]'::jsonb\n"
    assert normalized == prior.replace(removed, "")
    assert "jsonb_typeof(candidate_payload->'plan'->'delta_projections') = 'array'" in relaxed
    assert "candidate_payload->'authoritative_relation_witness'->'capture_origin'->>'capture_id' = capture_id::text" in relaxed
    assert "candidate_payload->'authoritative_relation_witness'->'operation_origin'->>'operation_id' = operation_plan_id::text" in relaxed
    assert "jsonb_path_exists(candidate_payload->'plan'->'delta_projections'" in relaxed
