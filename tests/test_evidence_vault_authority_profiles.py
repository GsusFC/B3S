...
def test_initial_matrix_is_complete_but_does_not_claim_semantic_calibration() -> None:
    ...
    assert other["new_semantic_mapping_profile_id"] == SEMANTIC_MAPPING_PROFILE_ID
    assert c8["new_semantic_mapping_profile_id"] == C8_PROFILE_ID
...
def test_c8_requires_human_for_new_mapping_but_accepts_a_reviewed_relation() -> None:
    ...
    assert c8_profile["automatic_new_mapping_enabled"] is False
    assert c8_profile["new_semantic_mapping_profile_id"] == C8_PROFILE_ID
    assert decision["eligible"] is True
...
def test_matrix_tampering_fails_closed() -> None:
    ...
    tampered["tile_profiles"][0]["automatic_new_mapping_enabled"] = True
