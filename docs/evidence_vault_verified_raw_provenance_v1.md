...
- the raw append atomically persists the exact pre-interpretation operation plan together with workspace/brand/scan/capture/evidence, watermark, receipts and evidence bindings; new plans may start only `pending` or `not_required`, while exact replay tolerates a legitimately evolved stored lifecycle;
