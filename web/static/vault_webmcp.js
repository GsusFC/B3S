(() => {
  ...
  function closedInput(value, allowedKeys, requiredKeys = []) {
    ...
    if (unknown.length) {
      throw new Error(`Campos no admitidos: ${unknown.join(", ")}.`);
    }
  ...
  function stringArg(
    input,
    key,
    { required = false, minLength = 0, maxLength = 4000, defaultValue = "" } = {}
  ) {
    ...
    if (normalized.length < minLength || normalized.length > maxLength) {
      throw new Error(
        `${key} debe tener entre ${minLength} y ${maxLength} caracteres.`
      );
    }
    return normalized;
  }
  ...
  function requireConfirmation(input, action) {
    if (input.confirm !== true) {
      throw new Error(
        `${action} requiere confirm=true tras una petición explícita del usuario.`
      );
    }
  }

  function dispatchValueChange(control) {
  ...
  function bounded(value, depth = 0) {
    ...
    if (Array.isArray(value)) {
      return value.slice(0, 100).map((item) => bounded(item, depth + 1));
    }
  ...
  function responseError(response, payload) {
    ...
    return new Error(
      compactText(
        detail || `B3S Vault respondió con HTTP ${response.status}.`,
        700
      )
    );
  }
  ...
  function assertSameOriginFinalUrl(response) {
    ...
    if (
      finalUrl.pathname.startsWith("/auth/") ||
      finalUrl.pathname === "/vault/review/login"
    ) {
      throw new Error(
        "La sesión requerida ha caducado. Inicia sesión en B3S Vault y repite la acción."
      );
    }
    return finalUrl;
  }
  ...
  async function listBrandAnalyses(rawInput, context = {}) {
    ...
    const limit = integerArg(input, "limit", {
      minimum: 1,
      maximum: 100,
      defaultValue: 25,
    });
    ...
    const matchingRows = query
      ? allRows.filter((row) =>
          [row.brandName, row.domain, row.url]
            .join(" ")
            .toLowerCase()
            .includes(query)
        )
  ...
  function findBrandSection(doc, label) {
    return [...doc.querySelectorAll(".brand-profile-block")].find(
      (section) =>
        nodeText(section.querySelector(".section-head .label"), 160) === label
    );
  }
  ...
  function parseBrandDocument(doc, requestedDomain) {
    ...
    const history = [...(historySection?.querySelectorAll("tbody tr") || [])]
      ...
      .map((row) => {
        ...
        return {
          ...
          publishable: !nodeText(cells[1], 200)
            .toLowerCase()
            .includes("diagnóstico"),
          canonicalStatus: nodeText(cells[2], 160),
          comparison: nodeText(cells[3], 160),
          reportId: nodeText(cells[4], 180),
          selectedSv9: [...(cells[5]?.querySelectorAll(".chip") || [])].some(
            (chip) => nodeText(chip, 80) === "SV9"
          ),
          latestAttempt: [...(cells[5]?.querySelectorAll(".chip") || [])].some(
            (chip) => nodeText(chip, 120) === "último intento"
          ),
        };
      });
    ...
    const reviewQueue = [
      ...doc.querySelectorAll("#cola-revision-vault table tbody tr"),
    ]
      .slice(0, 100)
      .map((row) => {
        const cells = [...row.querySelectorAll("td")];
    ...
    return {
      ...
      domain:
        nodeText(doc.querySelector(".brand-profile-domain-link"), 300) ||
        requestedDomain,
      url:
        doc.querySelector(".brand-profile-domain-link")?.getAttribute("href") ||
        "",
      ...
      currentReportId:
        currentReportPath.replace(/^\/report\//, "").split(/[?#]/)[0] || null,
      state: keyValues(stateSection?.querySelector("dl")),
      evidenceUrls: unique(
        [...(linksSection?.querySelectorAll("a[href]") || [])].map(
          (anchor) => anchor.href
        ),
        30
      ),
      ...
      vaultMemory: {
        ...
        reviewPath:
          doc
            .querySelector('a[href^="/vault/review/"]')
            ?.getAttribute("href") || null,
        contract: nodeText(
          vaultRoot?.querySelector(".vault-memory-contract"),
          1200
        ),
      },
    };
  ...
  async function getBrandAnalysis(rawInput, context = {}) {
    ...
    const { document: doc } = await fetchDocument(
      `/brand/${encodeURIComponent(domain)}?lang=es`,
      context.signal
    );
  ...
  function prepareScan(rawInput) {
    const input = closedInput(
      rawInput,
      ["url", "brandName", "allowDegradedFallback"],
      ["url"]
    );
    ...
    if (/[\u0000-\u001f\u007f]/.test(url)) {
      throw new Error("url no es válida.");
    }
    const brandName = stringArg(input, "brandName", { maxLength: 240 });
    const allowDegradedFallback = booleanArg(
      input,
      "allowDegradedFallback",
      false
    );

    const form = document.querySelector("form[data-b3s-scan-form]");
    const urlInput = form?.querySelector('input[name="url"]');
    const brandInput = form?.querySelector('input[name="brand_name"]');
    const fallbackInput = form?.querySelector(
      'input[name="allow_degraded_fallback"]'
    );
    if (!form || !urlInput || !brandInput || !fallbackInput) {
      throw new Error("El formulario visible de scan no está disponible.");
    }

    urlInput.value = url;
    dispatchValueChange(urlInput);
    brandInput.value = brandName;
    dispatchValueChange(brandInput);
    fallbackInput.checked = allowDegradedFallback;
    dispatchValueChange(fallbackInput);
    ...
    return {
      ...
      allowDegradedFallback: fallbackInput.checked,
    };
  }
  ...
  async function continueDegradedScan(rawInput, context = {}) {
    ...
    if (
      ...
      before?.acquisition_gate?.can_continue !== true
    ) {
      throw new Error(
        "El scan no está bloqueado en un estado que permita continuar degradado."
      );
    }
    const operation = await fetchJson(
      `/api/scan/${encodeURIComponent(scanId)}/continue`,
  ...
  async function cancelScan(rawInput, context = {}) {
    ...
    if (!["running", "blocked"].includes(before?.state)) {
      throw new Error(
        `El scan está en estado ${compactText(
  ...
  function parseReviewDocument(doc, domain, state) {
    ...
    const items = [...doc.querySelectorAll(".vault-review-card")]
      .slice(0, 100)
      .map((card) => {
        const columns = [
          ...card.querySelectorAll(".vault-review-grid > section"),
        ];
          ...
          sourceUrls: unique(
            [
              ...card.querySelectorAll(
                ".brand-profile-evidence-list a[href]"
              ),
    ...
    return {
      ...
      journalReady: ![...doc.querySelectorAll("p.error")].some((node) =>
        nodeText(node, 600).includes(
          "journal PostgreSQL no está disponible"
        )
      ),
      items,
    };
  ...
  async function listReviewCases(rawInput, context = {}) {
    ...
    if (!reviewDomain) {
      throw new Error("No hay un dominio de revisión asociado a esta página.");
    }
    const state = stringArg(input, "state", {
      maxLength: 20,
      defaultValue: "actionable",
    });
    if (!REVIEW_STATES.has(state)) throw new Error("state no es válido.");
    const currentState =
      new URLSearchParams(window.location.search).get("state") || "actionable";
    if (state === currentState) {
      return parseReviewDocument(document, reviewDomain, state);
    }
    const { document: doc } = await fetchDocument(
      `/vault/review/${encodeURIComponent(reviewDomain)}?state=${encodeURIComponent(
        state
      )}`,
      context.signal
    );
  ...
  function prepareReviewDecision(rawInput) {
    const input = closedInput(
      rawInput,
      ["caseId", "decision", "rationale"],
      ["caseId", "decision", "rationale"]
    );
    if (!reviewDomain) {
      throw new Error("No hay un dominio de revisión asociado a esta página.");
    ...
    const rationale = stringArg(input, "rationale", {
      required: true,
      minLength: 12,
      maxLength: 2000,
    });
    ...
    const form = card?.querySelector("form.vault-review-decision-form");
    if (!card || !form) {
      throw new Error("El caso no está visible o ya no admite una decisión.");
    }
    ...
    if (!radio || !textarea) {
      throw new Error("La decisión solicitada no está permitida para este caso.");
    }

    radio.checked = true;
    dispatchValueChange(radio);
  ...
  const flocTools = [
    {
      ...
      description:
        "Lista los análisis visibles en B3S Vault y permite filtrarlos por marca, dominio o URL. No ejecuta capturas ni modifica estado.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Devuelve score, estado, evidencia enlazada, historial y memoria Vault de una marca sin reanalizar ni cambiar autoridad.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Lee la exportación Markdown persistida de un informe B3S por ID. Devuelve contenido acotado y no ejecuta una nueva evaluación.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Consulta el estado, las fases y la adquisición de un scan existente de B3S Vault sin cambiarlo.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Prepara el formulario visible para un nuevo scan, pero no lo envía. Un miembro de FLOC debe revisar la URL y pulsar Run evidence scan para iniciar una adquisición con coste.",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description:
        "Continúa un scan bloqueado con adquisición degradada. Revalida el estado actual y exige confirm=true.",
      inputSchema: {
        type: "object",
        properties: {
          scanId: { type: "string", minLength: 1, maxLength: 180 },
    ...
    },
    {
      ...
      description:
        "Cancela un scan en ejecución o bloqueado. Revalida el estado actual y exige confirm=true porque interrumpe la operación.",
      inputSchema: {
        type: "object",
        properties: {
          scanId: { type: "string", minLength: 1, maxLength: 180 },
  ...
  const reviewTools = [
    {
      ...
      description:
        "Lista los casos de la revisión protegida actual con contrato de baldosa, cita, fuentes y estado. No registra decisiones.",
      inputSchema: {
        type: "object",
        properties: {
          state: {
    ...
    },
    {
      ...
      description:
        "Rellena una decisión y su justificación en el formulario protegido, pero no la envía ni la firma. El revisor humano mantiene el control final.",
      inputSchema: {
        type: "object",
        properties: {
          ...
          decision: {
            type: "string",
            enum: ["accepted", "disputed", "rejected"],
          },
  ...
  const tools =
    surface === "floc" ? flocTools : surface === "review" ? reviewTools : [];
  for (const tool of tools) {
    Promise.resolve(modelContext.registerTool(tool)).catch((error) => {
      console.warn(
        `[B3S Vault WebMCP] No se pudo registrar ${tool.name}.`,
        error
      );
    });
  }
